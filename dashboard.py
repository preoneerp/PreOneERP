...
with tabs[1]:
    ...
    if not df_p.empty:
        sel_v = st.selectbox("🔍 依供應商篩選", ["全部供應商"] + sorted(list(df_p['vendor'].unique())))
...
with tabs[2]:
    ...
    with st.container(border=True):
        ...
        plt_f = ["全部平台"] + sorted(list(df_o['platform'].unique()))
        sel_plt = c2.selectbox("📱 銷售平台", plt_f)
        mod_f = ["全部模式"] + sorted(list(df_o['mode'].unique()))
        sel_mod = c3.selectbox("🔃 交易模式", mod_f)
...
with tabs[3]:
    ...
    with st.container(border=True):
        ...
        sel_logi = l2.selectbox("🚚 物流商/渠道", ["全部物流"] + sorted(list(df_ent['logistics'].unique())))
