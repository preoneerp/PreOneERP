...
# --- 3. 數據自動處理 (修正 3/31 前資料消失的病灶) ---
def smart_process(df):
    if df is None or df.empty: return pd.DataFrame()
    df.columns = [str(c).lower().strip() for c in df.columns]
    
    # 關鍵：強制處理所有空值，避免舊資料在過濾 (Mask) 時被自動剔除
...
def fetch_all_data():
    try:
        # 分段抓取 3000 筆，確保 3/31 之前的資料被完整抓回
...
with tabs[2]:
    if not df_o.empty:
        ...
        if len(dr) == 2:
            # 關鍵修正：加入 na=False 防止 3/31 前資料因空值被剔除
...
with tabs[3]:
    if not df_o.empty:
        l_dr = st.date_input("📅 物流日期選擇", [date(2024, 3, 1), today])
        if len(l_dr) == 2:
