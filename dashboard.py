...
def smart_process(df):
    ...
    if t_col:
        ...
        # 關鍵修正：確保 pure_date 一定是日期物件類型，防止比較錯誤
...
with tabs[0]:
    ...
    # 修正點：計算前確保 pure_date 都是日期型態
    if not df_ship_all.empty:
        df_ship_all['pure_date'] = pd.to_datetime(df_ship_all['pure_date']).dt.date
        today_pkgs = pd.to_numeric(df_ship_all[df_ship_all['pure_date'] == today]['quantity'], errors='coerce').sum()
        month_pkgs = pd.to_numeric(df_ship_all[df_ship_all['pure_date'] >= this_month]['quantity'], errors='coerce').sum()
    else: today_pkgs = month_pkgs = 0
...
with tabs[2]:
    if not df_o.empty:
        ...
        if len(dr) == 2:
            df_o['pure_date'] = pd.to_datetime(df_o['pure_date']).dt.date
    ...
    if not df_o.empty:
        ...
        if len(l_dr) == 2:
            ...
            if not df_entry.empty:
                df_entry['pure_date'] = pd.to_datetime(df_entry['pure_date']).dt.date
