...
with tabs[0]:
    ...
    with m3: st.markdown(f'<div class="metric-card"><div class="metric-label">今日商品明細筆數</div><div class="metric-value">{len(df_items)} 筆</div></div>', unsafe_allow_html=True)
    
    st.markdown("#### 🚚 今日物流管道分佈")
    if not df_ship_summary.empty:
        logi_display = df_ship_summary.groupby('logistics')['quantity'].sum().reset_index()
        logi_display.columns = ['物流方式', '件數']
        st.dataframe(logi_display, use_container_width=True, hide_index=True)
    else:
        st.info("今日尚無物流資料")
