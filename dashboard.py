...
def smart_process(df):
    ...
    # 2. 解決空值：將所有空欄位補上「未分類」，防止過濾器因為 NaN 而丟棄資料
    for col in df.columns:
        if df[col].dtype == 'object':
            df[col] = df[col].astype(str).str.strip().replace(['nan', 'None', 'None', ''], '未分類')
    
    # 3. 解決時間對齊：暴力搜尋所有可能的時間欄位
    t_col = next((c for c in df.columns if c in ['timestamp', 'time', 'created_at', '作成時間', '日期']), None)
    if not t_col:
        for col in df.columns:
    ...
    if t_col:
        ...
        # 如果解析失敗，給予一個保底日期，確保不會在日期篩選時消失
        df['tz_fixed'] = df['tz_fixed'].fillna(pd.Timestamp('2024-03-01'))
        df['pure_date'] = df['tz_fixed'].dt.date
    
    # 4. 解決 Key 缺失：修復 vendor 欄位遺失問題
    if 'vendor' not in df.columns:
        v_alt = next((c for c in df.columns if '供應商' in c or 'v_name' in c), None)
        df['vendor'] = df[v_alt] if v_alt else "未分類"
    return df
...
# --- 5. 數據抓取 (翻頁讀取：確保抓到被擠掉的舊資料) ---
@st.cache_data(ttl=10)
def fetch_all_data():
    try:
        # 分三段抓取 3000 筆，解決 Supabase 1000 筆限制
...
raw_p_df, raw_o_df = fetch_all_data()
df_p, df_o = smart_process(raw_p_df), smart_process(raw_o_df)
...
with tabs[0]:
    ...
    # 核心修正：使用 na=False 且不進行「!= 物流登記」的硬性排除，改用包含式排除
...
with tabs[2]:
    if not df_o.empty:
        dr = st.date_input("📅 日期篩選", [date(2024, 3, 1), today]) # 起始日期預設拉到 3 月
        if len(dr) == 2:
            # 兼容性篩選：使用 na=False，確保 3/31 前那些欄位不齊全的資料，不會因為篩選而消失
...
with tabs[3]:
    ...
    if st.button("🔄 刷新數據庫", use_container_width=True): st.cache_data.clear(); st.rerun()
