...
# --- 1. 頁面配置與視覺設計 ---
st.set_page_config(page_title="培玩雲端 ERP WEB V0407.7", layout="wide", initial_sidebar_state="expanded")
...
with tabs[0]:
    ...
    p_cols = st.columns(6)
    df_items = today_o[~today_o['p_name'].str.contains("物流|包裹", na=False)] if not today_o.empty else pd.DataFrame()
    ...
    for i, item in enumerate(prods):
        with p_cols[i]:
            ...
            if not df_items.empty:
                # 使用更加寬鬆的關鍵字比對，避開編號干擾
                qty = int(df_items[df_items['p_name'].str.contains(item['s'], na=False)]['quantity'].sum())
    ...
    with col_m:
        st.markdown("#### 📈 今日核心指標")
        df_ship = df_o[df_o['p_name'].str.contains("物流|包裹", na=False)] if not df_o.empty else pd.DataFrame()
        ...
        low_s = len(df_p[pd.to_numeric(df_p['stock'], errors='coerce') < 10]) if not df_p.empty else 0
        ...
        m4.markdown(f'<div class="metric-card"><div class="metric-label">系統版本</div><div class="metric-value" style="font-size:1.1rem; color:#27AE60;">V0407.7 鋼鐵版</div></div>', unsafe_allow_html=True)
...
with tabs[1]:
    if not df_p.empty:
        ...
        st.dataframe(f_p, use_container_width=True, hide_index=True)
...
with tabs[3]:
    st.markdown("### 🚚 物流件數登記")
    df_ent = df_o[df_o['p_name'].str.contains("物流|包裹", na=False)] if not df_o.empty else pd.DataFrame()
