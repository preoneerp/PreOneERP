...
# --- 3. 數據預處理 (修復 ValueError 核心邏輯) ---
def smart_process(df):
    if df is None or df.empty: return pd.DataFrame()
    # 統一小寫
    df.columns = [str(c).lower().strip() for c in df.columns]
    
    # 時間轉換：加入 errors='coerce' 防止格式不一導致崩潰
    if 'timestamp' in df.columns:
        # 使用 coerce 會將無法辨識的格式轉為 NaT
        df['tz_fixed'] = pd.to_datetime(df['timestamp'], utc=True, errors='coerce')
        # 轉換時區並處理空值
