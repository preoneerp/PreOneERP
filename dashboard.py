...
with tabs[2]:
    if not df_o.empty:
        with st.container(border=True):
            ...
            dr = cc1.date_input("📅 日期範圍", [date(2024, 3, 1), today]) # 起始設回 3 月
...
with tabs[3]:
    if not df_o.empty:
        with st.container(border=True):
            ...
            l_dr = lc1.date_input("📅 物流日期", [date(2024, 3, 1), today])
