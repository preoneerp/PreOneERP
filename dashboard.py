...
with tabs[0]:
    ...
    cols = st.columns(6)
    # 重要：計算熱銷標籤時，僅抓 mode 為 "出貨" 的資料
    df_only_out = today_o[today_o['mode'] == '出貨']
    for i, item in enumerate(target_prods):
        with cols[i]:
            qty = int(df_only_out[df_only_out['p_name'] == item['search']]['quantity'].sum())
    ...
    # 【關鍵修正】：件數登記資料抓取 (同時兼容 "物流登記" 名稱與 "物流統計" 模式)
    df_ship_recs = today_o[(today_o['p_name'] == "物流登記") | (today_o['mode'] == "物流統計")]
    today_total_pkgs = df_ship_recs['quantity'].sum()
    ...
    with m3: st.markdown(f'<div class="metric-card"><div class="metric-label">今日作業明細筆數</div><div class="metric-value">{len(today_o[~today_o["p_name"].isin(["物流登記"])])} 筆</div></div>', unsafe_allow_html=True)
    
    st.markdown("#### 🚚 今日物流分佈統計")
    if not df_ship_recs.empty:
        logi_summary = df_ship_recs.groupby('logistics')['quantity'].sum().reset_index()
...
with tabs[2]:
    ...
    dr = cc1.date_input("📅 日期", [today, today])
    ...
    st.dataframe(final_o[['tz_fixed', 'p_name', 'quantity', 'mode', 'platform', 'logistics']].rename(columns={'tz_fixed':'時間','p_name':'商品名稱'}), use_container_width=True, hide_index=True)

# --- TAB 3: 物流登記歷史 ---
with tabs[3]:
    st.success("🚚 以下為電腦版「過帳包裹件數」的完整歷史紀錄")
    # 這裡同樣兼容兩種標記方式
    df_l = df_o[(df_o['p_name'] == "物流登記") | (df_o['mode'] == "物流統計")].copy()
    if not df_l.empty:
        df_l = df_l.sort_values('tz_fixed', ascending=False)
        st.dataframe(df_l[['tz_fixed', 'platform', 'logistics', 'quantity']].rename(columns={'tz_fixed':'時間','quantity':'件數'}), use_container_width=True, hide_index=True)
    
    if st.button("🔄 立即重新整理雲端數據", use_container_width=True):
        st.cache_data.clear()
        st.rerun()
