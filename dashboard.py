...
def smart_process(df):
    ...
    # 關鍵：強制將所有 Object 欄位轉字串並處理空值，防止過濾器誤殺
...
with tabs[2]:
    if not df_o.empty:
        dr = st.date_input("📅 日期篩選", [date(2024, 3, 1), today])
        if len(dr) == 2:
            # 關鍵修復：加入 na=False，防止 3/31 前資料因空值被剔除
...
with tabs[3]:
    if not df_o.empty:
        ...
        if len(l_dr) == 2:
            ...
            st.markdown(f'<div style="background:#E67E22; color:white; padding:15px; border-radius:15px; text-align:center; margin-bottom:20px">總包裹數: {int(df_entry[e_mask]["quantity"].astype(float).sum())} 件</div>', unsafe_allow_html=True)
